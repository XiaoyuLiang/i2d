...
##
codeGCluster <- function(dat=dat, wt=4, k=NULL, diff_distance=0.1, method="louvain"){
  ...
  if(diff_distance<=0 | is.na(diff_distance)){
    diff_distance=0.5
  }
  ...
  if (is.null(k) | in.g){
    ...
    cat(paste0("do graph clustering using ",method," method...\n"))
    if (method=="louvain"){
      ...
      stop("The method should be selected from louvain, fast_greedy, infomap, label_prop or spinglass.\n")
    }
